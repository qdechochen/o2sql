...
class Command extends Ast {
  async execute() {
    throw new Error(
      '[o2sql] execute handler is not supported directly since v4. Please use o2sql-pg to integrate with pg, or use your own logic.'
    );
  }
}
...
class Select extends BaseCommand {
      ...
      if (typeof t === 'object' && t.table) {
        (t.fields || t.columns).forEach(p => {
          ...
          if (typeof p === 'string') {
            ...
            p[1] = getAliasName(t.prefix || t.table, t.separator, p[1] || p[0]);
  ...
  distinct(distinct = []) {
    ...
    this.data.distinct = [];
    fields.forEach(t => {
      this.data.distinct.push(new IdentifierAst(t));
    });
  ...
  groupby(groupby) {
    ...
    groupby.forEach(t => {
      this.data.groupby.push(new IdentifierAst(t));
    });
  ...
  toParams(params, { parentheses } = {}) {
    ...
    if (this.data.distinct) {
      if (this.data.distinct.length === 0) {
        ...
        params.sql += 'DISTINCT ON (';
        this.data.distinct.forEach((t, index) => {
          t.toParams(params, { separator: index === 0 ? '' : ',' });
        });
        params.sql += ') ';
      }
    }
    ...
    if (this.data.from) {
      ...
      if (this.data.having) {
        params.sql += ' HAVING ';
        this.data.having.toParams(params);
      }
