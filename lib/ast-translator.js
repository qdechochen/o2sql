...
function resolveValue(value) {
  ...
  if (value === null) {
    ...
    ast = {
      type: 'expr_list',
      value: value.map(t => ({ type: typeof t, value: t })),
    };
...
function resolveField(column) {
  ...
  if (typeof column === 'object' && !(column instanceof Array)) {
    const { table, prefix, separator, fields } = column;
    fields.forEach(field => {
      ...
      columns.push({
        ...
        alias: prefix
          ? prefix +
            (separator
              ? separator + subColumn.alias
              : capFirstLetter(subColumn.alias))
          : subColumn.alias,
      });
    });
  }
  ...
  if (typeof column === 'string') {
    ...
    if (column.___className === 'Command') {
      column = column.ast;
    }
...
function resolveJoinTable(table, isRight = false) {
  ...
  if (table.name.___className === 'Command') {
    if (!table.alias) {
      table.alias = Math.random()
        .toString(36)
        .substr(2);
    }
    return {
...
function resolveJoin(join, parentheses = false) {
  ...
  if (join.on) {
    if (typeof join.on === 'string') {
      ...
      joinon = resolveWhereAndOr('$and', join.on)
    } else {
      throw new Error('[O2SQL] incorrect "on" clause');
    }
  } else {
    const [ltable, lcol = 'id'] = join.left.key
      ? resolveColumnName(join.left.key)
      : [];
    const [rtable, rcol = 'id'] = join.right.key
      ? resolveColumnName(join.right.key)
      : [];
    joinon = {
      type: 'binary_expr',
      operator: '=',
...
function resolveWhereAndOr(op, where, parentheses = false) {
  ...
  if (keys.length === 1) {
    if (keys[0].startsWith('$and') || keys[0].startsWith('$or')) {
  ...
  return {
    type: 'binary_expr',
    operator: op.startsWith('$or') ? 'OR' : 'AND',
    left:
      first.startsWith('$and') || first.startsWith('$or')
        ? resolveWhereAndOr(first, firstNode, true)
        : resolveWhereNode(first, firstNode),
...
function resolveWhereNodeValuePair(key, op, value) {
  ...
  if (typeof key === 'string') {
    ...
    left = resolveValue(key);
  }

  const right = resolveValue(value);
  /*
  if (value && value.___className === 'Command') {
    value = value.ast;
  }
...
function resolveWhereNode(key, value) {
  ...
  if (value instanceof Array) {
    if (value.find(t => t !== null && !['number', 'string', 'boolean'].includes(typeof t))) {
      throw new Error('[O2SQL] only number, string, boolean element is accepted in "IN" clause');
    }
  ...
  if (typeof value === 'object' && value.___className !== 'Command') {
...
module.exports = {
  func(name, ...args) {
    const ast = {
      ...
      args: {
        type: 'expr_list',
        value: args.map(t => resolveValue(t)),
      },
    };
    return ast;
  },
  ...
  identifier(name) {
    const [table, column] = resolveColumnName(name);
    return {
      type: 'column_ref',
      table,
      column,
  ...
  columns(columns) {
    ...
    columns.forEach(item => {
      ...
      fields.forEach(field => {
        const { table, expr, column, alias, castas } = field;
        let astColumn;
        if (expr) {
          astColumn = {
            expr,
    ...
    values.forEach(row => {
      ...
      astColumns.forEach(field => {
        ...
        if (
          val &&
          typeof val === 'object' &&
          ['expr_list', 'select'].includes(val.type)
        ) {
          rowValues.push(val);
        } else {
          rowValues.push({
  ...
  set(values) {
    ...
    Object.keys(values).forEach(key => {
      ...
      if (
        val &&
        typeof val === 'object' &&
        ['expr_list', 'select', 'function', 'count'].includes(val.type)
      ) {
        ast.push({
          column: key,
          value: val,
