...
function parseExprObject(oexpr) {
  if (oexpr instanceof ExprAst) return oexpr;
  let ast;
  if (oexpr.$left) {
...
class ExprListAst extends Ast {
  ...
  toParams(params = { sql: '', values: [] }, { parentheses = false } = {}) {
    if (parentheses) {
      params.sql += '(';
    }
    params.sql += 'ARRAY[';
...
class FunctionAst extends Ast {
  ...
  toParams(params = { sql: '', values: [] }) {
    params.sql +=
      joinNameParts(this.db, this.schema, this.table, this.name) + '(';
...
class IdentifierAst extends Ast {
  ...
  toParams(params = { sql: '', values: [] }, { separator = '' } = {}) {
