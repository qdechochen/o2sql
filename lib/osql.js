...
osqlFactory.insert = values => osqlFactory('insert').values(values);
osqlFactory.insertInto = table => osqlFactory('insert').into(table);
osqlFactory.update = table => osqlFactory('update').table(table);
osqlFactory.delete = table => osqlFactory('delete').from(table);
osqlFactory.deleteFrom = osqlFactory.delete;

module.exports = osqlFactory;
