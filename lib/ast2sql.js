...
class AST2Sql {
  ...
  binaryToSQL(expr) {
    ...
    let rstr = this.exprToSQL(expr.right);
    ...
    const str = this.exprToSQL(expr.left) + ' ' + operator + ' ' + rstr;
  ...
  tablesToSQL(from) {
    ...
    let str = baseTable.table ? identifierToSql(baseTable.table) : this.exprToSQL(baseTable.expr);
    ...
    for (let i = 1; i < tables.length; i++) {
      ...
      if (joinExpr.table) {
        ...
        str += this.exprToSQL(joinExpr.expr);
      }

      if (joinExpr.as !== null) str += ' AS ' + identifierToSql(joinExpr.as);
      if (has(joinExpr, 'on') && joinExpr.on !== null) str += ' ON ' + this.exprToSQL(joinExpr.on);
  ...
  selectToSQL(stmt) {
    ...
    if (Array.isArray(stmt.limit)) clauses.push('LIMIT', stmt.limit.map(this.exprToSQL));
  ...
  updateToSQL(stmt) {
    ...
    if (stmt.table) clauses.push(identifierToSql(stmt.table));
    if (stmt.set) {
      clauses.push('SET', stmt.set.map(value => identifierToSql(value.column) + '=' + this.literalToSQL(value.value)).join(', '));
    }
    if (has(stmt, 'where') && stmt.where !== null) clauses.push('WHERE ' + this.exprToSQL(stmt.where));
    if (stmt.returning) clauses.push('RETURNING', this.columnsToSQL(stmt.returning));
    return clauses.join(' ');
