...
class AST2Sql {
  ...
  binaryToSQL(expr) {
    ...
    if (expr.right.type === 'null') {
      ...
      if (operator === 'IN' || operator === 'NOT IN') {
        ...
        if (
          !itemType &&
          expr.right.value[0].type === 'number' &&
          ~~expr.right.value[0].type === expr.right.value[0].type
        ) {
          itemType = 'INTEGER';
        }
        rstr = this.exprToSQL({
   ...
   */
  tablesToSQL(from) {
    if (from.type === 'table_ref' || from.type === 'table_ast') {
  ...
  toParams(values) {
    ...
    if (
      !['select', 'count', 'insert', 'update', 'delete'].includes(this.ast.type)
    ) {
