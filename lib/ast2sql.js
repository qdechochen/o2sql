...
class AST2Sql {
  ...
  literalToSQL(literal, transformArray = true) {
    // const { type } = literal;
    ...
    */
    ...
    if (type === 'array') {
      ...
      if (transformArray) {
        index = 'ARRAY[' + value.map((t, i) => `$${start + i}`).join(',') + ']' + (typeof value[0] === 'string' ? '::VARCHAR[]' : '');
      } else {
        index = value.map((t, i) => `$${start + i}`).join(',');
      }
      /*
  ...
  exprToSQL(expr, transformArray = true) {
    return this.exprToSQLConvertFn[expr.type]
      ? this[this.exprToSQLConvertFn[expr.type]](expr) : this.literalToSQL(expr, transformArray);
  }
  ...
  columnRefToSQL(expr) {
    ...
    const table = (has(expr, 'table') && expr.table !== null ? expr.table : this.ast.mainTable) || null;
  ...
  columnsToSQL(columns) {
    ...
    return columns
      .map((column) => {
        let str;
        if (typeof column.expr.column === 'object' && column.expr.column.type === 'select') {
  ...
  selectToSQL(stmt) {
    ...
    if (has(stmt, 'distinct') && stmt.distinct instanceof Array) {
      if (stmt.distinct.length === 0) {
        ...
        clauses.push('distinct on (', this.getExprListSQL(stmt.distinct).join(', '), ')');
      }
    }
  ...
  insertToSQL(stmt) {
    ...
    clauses.push('VALUES');
    if (stmt.values) {
      clauses.push(stmt.values.map(row => '(' + row.value.map((value) => {
        let sql;
