...
class AST2Sql {
    ...
    */
    ...
    if (type === 'array') {
      ...
      if (transformArray) {
        index =
          'ARRAY[' +
          value.map((t, i) => `$${start + i}`).join(',') +
          ']' +
          (typeof value[0] === 'string' ? '::VARCHAR[]' : '');
      } else {
  ...
  exprToSQL(expr, transformArray = true) {
    return this.exprToSQLConvertFn[expr.type]
      ? this[this.exprToSQLConvertFn[expr.type]](expr)
      : this.literalToSQL(expr, transformArray);
  }
  ...
  tablesToSQL(from) {
    ...
    const lstr =
      from.left.type === 'table_join'
        ...
        : identifierToSql(from.left.table) +
        (from.left.as ? ' ' + identifierToSql(from.left.as) : '');

    const rstr =
      from.right.type === 'table_join'
        ? this.tablesToSQL(from.right)
        : identifierToSql(from.right.table) +
        (from.right.as ? ' ' + identifierToSql(from.right.as) : '');
